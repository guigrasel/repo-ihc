...
(function (global, factory) {
  ...
  function useQuery(opts) {
      ...
      if (vue.isRef(query)) {
          vue.watch(query, executeIfNotPaused);
      }
      if (isWatchable(paused)) {
          vue.watch(() => !isCurrentlyPaused(), shouldExecute => {
      ...
      function initVarWatchers() {
          ...
          vue.watch(() => unwrap(variables), newValue => {
  ...
  function useSubscription(opts, reduce = defaultReducer) {
      ...
      const error = vue.ref(null);
      const isPaused = vue.computed(() => unravel(paused, variables));
      ...
      const vm = vue.getCurrentInstance();
      if (!isPaused.value) {
          vm ? vue.onMounted(initObserver) : initObserver();
      }
      // TODO: if outside of setup, it should be recommend manually pause it(or some action else)
      vm && vue.onBeforeUnmount(() => observer === null || observer === void 0 ? void 0 : observer.unsubscribe());
      if (isWatchable(paused)) {
          vue.watch(paused, val => {
              if (!val) {
                  initObserver();
              }
          });
      ...
      if (vue.isRef(query)) {
          vue.watch(query, initObserver);
      }
      if (vue.isRef(variables)) {
          vue.watch(variables, initObserver);
      }
  ...
  exports.mergeFetchOpts = mergeFetchOpts;
