...
function useQuery(opts) {
  ...
  async function execute2(overrideOpts) {
  ...
  initVarWatchers();
  const api = { data, isFetching, isDone, error, execute: execute2 };
  const vm = getCurrentInstance();
  if (currentFetchOnMount) {
    if (!paused || !isCurrentlyPaused()) {
      vm ? onMounted(() => execute2()) : execute2();
    }
  }
...
function useMutation(query, opts) {
  ...
  const error = ref(null);
  let lastPendingOperation;
  async function execute2(variables) {
    isFetching.value = true;
  ...
  return { data, isFetching, isDone, error, execute: execute2 };
}
var defaultReducer = (_, val) => val.data;
